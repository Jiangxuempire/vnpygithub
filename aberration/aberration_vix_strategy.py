...
class AberrationStrategy(CtaTemplate):
    ...
    def on_xmin_bar(self, bar: BarData):
        ...
        dev = abs(self.am.close[:-1] - self.boll_mid_array[:-1]) / std_array[:-1]
        dev_max = dev[-self.boll_length:].max()
        boll_up_array = self.boll_mid_array + std_array * dev_max
        boll_down_array = self.boll_mid_array - std_array * dev_max
