...
class AdaptStrategyV1(CtaTemplate):
    """
    策略逻辑：
    一、、过虑信号  （小时周期）
    使用rsiboll来判断信号强弱
    ...
    def on_minute_bar(self, bar: BarData):
        ...
        if not self.am_open.inited or not self.am_hour.inited:
            return
        ...
        if not self.pos:
            ...
            ema_mid_new = self.am_hour.sma(self.ema_length_new, True)
    ...
    def on_hour_bar(self, bar: BarData):
        ...
        if not self.am_hour.inited:
            return
        #
        # ema_array = talib.SMA(self.am_hour.close, self.rsi_length)
        # std_array = talib.STDDEV(self.am_hour.close, self.rsi_length)
        #
        # dev_array = abs(self.am_hour.close - ema_array) / std_array
        # dev_max = talib.MAX(dev_array, self.rsi_length)

        rsi_array = talib.RSI(self.am_hour.close, self.rsi_length)
        ...
        rsi_up_array = rsi_ma + rsi_std * dev_max
        rsi_dow_array = rsi_ma - rsi_std * dev_max

        print(rsi_up_array)
        print(rsi_dow_array)
        exit()

        rsi_value_array = self.am_hour.rsi(self.rsi_length, True)
