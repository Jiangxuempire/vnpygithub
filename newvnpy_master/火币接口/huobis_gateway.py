...
# 条件单 返回 2:已提交、4:报单成功、5:报单失败、6:已撤单
# 限价单返回	订单状态(1准备提交 2准备提交 3已提交 4部分成交 5部分成交已撤单 6全部成交 7已撤单 11撤单中)
STATUS_HUOBIS2VT: Dict[int, Status] = {
    3: Status.NOTTRADED,
    4: Status.PARTTRADED,
    5: Status.PARTTRADED_CANCELLED,
    6: Status.ALLTRADED,
    7: Status.CANCELLED,
}

STATUS_HUOBIS2VT_STOP: Dict[int, Status] = {
    2: Status.NOTTRADED,
    4: Status.SUCCESS,
    5: Status.FAIL,
    6: Status.CANCELLED,
...
ORDERTYPE_VT2HUOBIS: Dict[OrderType, Any] = {
    ...
    # OrderType.OPTIMAL_10: "optimal_10",
    # OrderType.OPTIMAL_20: "optimal_20",
}
...
ORDERTYPE_HUOBIS2VT_OPTIMAL_5 = "optimal_5"
...
class HuobisGateway(BaseGateway):
    ...
    def connect(self, setting: dict) -> None:
        ...
        self.trade_ws_api.connect(key, secret, WEBSOCKET_TRADE_HOST, proxy_host, proxy_port)
        self.market_ws_api.connect(key, secret, WEBSOCKET_DATA_HOST, proxy_host, proxy_port)
    ...
    def send_order(self, req: OrderRequest) -> str:
        ...
        if req.type == OrderType.STOP:
            ...
            data = {
                ...
                "order_price_type": ORDERTYPE_HUOBIS2VT_OPTIMAL_5,
        ...
        # print(f"order.vt_orderid: {order.vt_orderid}")
        return order.vt_orderid

    def send_orders(self, reqs: Sequence[OrderRequest]) -> list:
        """"""
        orders_data = []
        orders = []
        vt_orderids = []
...
class HuobisTradeWebsocketApi(HuobisWebsocketApiBase):
    ...
    def subscribe_trigger_order(self):
        """"""
        self.req_id += 1
        req = {
            "op": "sub",
            "cid": str(self.req_id),
            "topic": f"trigger_order.*"
        }
        self.send_packet(req)
    ...
    def on_data(self, packet) -> None:
        ...
        elif "trigger_order" in topic:
            self.on_stop_order(packet)

    def on_stop_order(self, data: dict) -> None:
        """
        """
        ...
        for d in orders:
            ...
            order = OrderData(
                ...
                direction=DIRECTION_HUOBIS2VT[d["direction"]],
                offset=OFFSET_HUOBIS2VT[d["offset"]],
                price=d["trigger_price"],
                volume=d["volume"],
                # traded=d["trade_volume"],
                status=STATUS_HUOBIS2VT_STOP[d["status"]],
                datetime=dt,
                gateway_name=self.gateway_name,
            )
            # print(f'd["order_id"]:{d["order_id"]}')
            self.gateway.on_order(order)
