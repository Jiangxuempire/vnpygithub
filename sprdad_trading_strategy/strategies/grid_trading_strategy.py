...
class GridTradingStrategy(SpreadStrategyTemplate):
    ...
    def on_spread_bar(self, bar: BarData):
        ...
        if long_volume > 0:
            ...
            self.start_long_algo(price=long_price, volume=abs(long_volume), payup=5, interval=5)

        # 做空，检查最大持仓，和当前持仓的差值
        short_volume = self.max_target - self.spread_pos
        if short_volume < 0:
            short_price = bar.close_price - self.pay_up
            self.start_short_algo(price=short_price, volume=abs(short_volume),payup=5, interval=5)
